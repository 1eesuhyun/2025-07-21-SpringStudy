...
public class DataBoardController {
	...
	{
		// Model model : JSP / HTML 로 데이터전송 객체 = request대체
		if(page==null)
			page="1";
		int curpage=Integer.parseInt(page);
		int rowSize=10;
		int startPage=(curpage-1)*rowSize; // OFFSET => 0 , rownum => 1
		List<DataBoardVO> list=dservice.dataBoardList(curpage);
	 ...
	 *    => Cookie / Session(HttpSession session)
	 *        | request/response
	...
	@ResponseBody
	// 화면 이동 => HTML/JavaScript에 필요한 데이터를 전송
	// 단점 : 한글이 깨진다
